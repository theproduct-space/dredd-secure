...
export default function useIbcCoreClientV1() {
  ...
  const QueryClientStates = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery(
      ...
      ({ pageParam = 1 }: { pageParam?: number }) => {
        ...
        return client.IbcCoreClientV1.query
          .queryClientStates(query ?? undefined)
  ...
  const QueryConsensusStates = (
    ...
    return useInfiniteQuery(
      ...
      ({ pageParam = 1 }: { pageParam?: number }) => {
        ...
        query["pagination.count_total"] = true;
        return client.IbcCoreClientV1.query
          .queryConsensusStates(client_id, query ?? undefined)
          .then((res) => ({ ...res.data, pageParam }));
      },
      {
        ...options,
  ...
  const QueryConsensusStateHeights = (
    client_id: string,
    query: any,
    options: any,
    perPage: number,
    ...
    return useInfiniteQuery(
      ...
      ({ pageParam = 1 }: { pageParam?: number }) => {
        ...
        query["pagination.count_total"] = true;
        return client.IbcCoreClientV1.query
          .queryConsensusStateHeights(client_id, query ?? undefined)
          .then((res) => ({ ...res.data, pageParam }));
      },
      {
        ...options,
